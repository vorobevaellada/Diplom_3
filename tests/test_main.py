...
class TestMainFunctions:
    ...
    def test_ingredient_popup(self, driver):
        ...
        main.wait_bun()
        main.click_bun()
        # Нужен ли отдельный класс для popup
    ...
    def test_add_ingredient(self, driver):
        """Тест проверки добавления ингредиента в заказ и появления соответствующего счетчика на главной странице"""
        main = MainPage(driver)
        main.get_url(Urls.MAIN_PAGE)
        main.ingredient_drag_and_drop()
        main.wait_counter_visible()
        assert driver.find_element(*MainPageLocators.FLUOR_BUN_COUNTER).is_displayed()